...
from scripts.dbutils.minioutils import upload_csv_from_bytes, get_object_list, get_object_content
from scripts.datautils.dateutils import get_last_execution_date, set_last_execution_date
from scripts.dbutils.mongoutils import insert_dataframe_to_mongo
...
def run(base_parameters):
    ...
    client = base_parameters.get("client")
    mongo_database = base_parameters.get("mongo_database")
    ...
    # Get object list from last execution onward
    ...
    for obj in objects:
        ...
        df_raw = pd.read_parquet(obj_data)
        ...
        date = latest_date.strftime('%Y-%m-%d %H%M%S.%f')
        ...
        try:
            ...
            csv_buffer.seek(0)
            file_name = f"{target_bucket}_app{datename}.csv"
            upload_csv_from_bytes(csv_buffer, f"{target_path}{file_name}")

            # TODO: MongoDB indexing/additional processing

            # Upload to MongoDB
            insert_dataframe_to_mongo(client, mongo_database, target_bucket, df)
            ...
            raise Exception(f"APP STORAGE ERROR: {e}")